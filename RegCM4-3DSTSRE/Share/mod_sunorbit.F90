...
module mod_sunorbit
  ...
  public :: sunmodi  !ChlGu AnHuang Add 3DSTSRE
  contains
  !
  ...
  Subroutine sunmodi(jday,lat,lon,declin,orb_cosz1,orb_cosz_gu,sun_Az,xxugg,xxvgg,xxwgg)
    implicit none
    real(rk8) , intent(inout) :: orb_cosz1 !coszrs..
    real(rk8) , intent(inout) :: orb_cosz_gu !cosine of the sloar-beam incident angle over rugged area.
    real(rk8) , intent(inout) ::  sun_Az ! solar azimuth
    real(rk8) , intent(in) :: jday ! Julian cal day
    real(rk8) , intent(in) :: lat ! Centered latitude (radians)
    real(rk8) , intent(in) :: lon ! Centered longitude (radians)
    real(rk8) , intent(in) :: declin ! Solar declination (radians)
    real(rk4) , intent(in) ::xxugg,xxvgg,xxwgg !terrain modification
    real(rk8)   :: tmphour

    orb_cosz1 = sin(lat)*sin(declin) - &
               cos(lat)*cos(declin)*cos(jday*2.0D0*mathpi+lon) 
  
      


     if(orb_cosz1.eq.1.0) sun_Az=mathpi

     if(sin(jday*2.0D0*mathpi+lon).gt.0.0) sun_Az=acos((sin(declin)&
      &-orb_cosz1*sin(lat))/(cos(lat)*sqrt(1.0-orb_cosz1**2)))
     
    if(sin(jday*2.0D0*mathpi+lon).lt.0.0) sun_Az=2*mathpi-&
     &acos((sin(declin)-orb_cosz1*sin(lat))&
       &/(cos(lat)*sqrt(1.0-orb_cosz1**2)))
 
   orb_cosz_gu = xxugg*orb_cosz1&
    &+xxvgg*sqrt(1- orb_cosz1**2)*cos(sun_Az)&
     &+ xxwgg*sqrt(1- orb_cosz1**2)*sin(sun_Az)   
 !   orb_cosz_gu=orb_cosz1

!  print*,'hourangle=',(jday*2.0D0*mathpi+lon),'cosi=',orb_cosz_gu,'coszrs=',orb_cosz1,'sun_Az=',sun_Az
  end Subroutine sunmodi
  ...
  subroutine orb_decl(calday,eccen,mvelpp,lambm0,obliqr,delta,eccf)
    ...
    lambm = lambm0 + (calday - vernal_equinox)*2.D0*mathpi/dayspy
    lmm   = lambm  - mvelpp
