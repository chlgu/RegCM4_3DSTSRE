...
module mod_params
 ...
 !ChlGu AnHuang Add 3DSTSRE 20210215
  integer(ik4) chlguii
  integer(ik4)  jgu,igu  !,jgu1,igu1 !ChlGu AnHuang Add 3DSTSRE 20210407
...
!if ( myid == 3 ) then
  ...
  !ChlGu AnHuang Add 3DSTSRE 20210408
       allocate(mddom%ugg(jde1:jde2,ide1:ide2))
       allocate(mddom%vgg(jde1:jde2,ide1:ide2))
       allocate(mddom%wgg(jde1:jde2,ide1:ide2))
       allocate(mddom%SFgg(jde1:jde2,ide1:ide2,1:360,1:101))
       allocate(mddom%asvfgg(jde1:jde2,ide1:ide2))
       allocate(mddom%refgg(jde1:jde2,ide1:ide2))

  !ChlGu AnHuang Add 3DSTSRE 20210917 
       allocate(mddom%TACBgg(jde1:jde2,ide1:ide2))
       allocate(mddom%TASBgg(jde1:jde2,ide1:ide2))
       allocate(mddom%SECAgg(jde1:jde2,ide1:ide2))
...
!ChlGu AnHuang Add 3DSTSRE 20210408
       open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/uvwxy/&
        &TP_u_SRTM_IDW_distributed.grd'&
               &,form='unformatted'&
                 &,access='direct',recl=4&
                  &,action='read',status='old',convert='little_endian')                  
                    !read(116,rec=1)mddom%ugg
        	do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
                      read(116,rec=(igu-1)*210+jgu) mddom%ugg(jgu,igu)
                 enddo
				enddo
       close(116)
     ! print*,'uuuuugggggg=',mddom%ugg

       open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/uvwxy/&
        &TP_v_SRTM_IDW_distributed.grd'&
               &,form='unformatted'&
                 &,access='direct',recl=4&
                  &,action='read',status='old',convert='little_endian')                  
				do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
                      read(116,rec=(igu-1)*210+jgu) mddom%vgg(jgu,igu)
                 enddo
				enddo
       close(116)
       ...
       open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/uvwxy/&
        &TP_w_SRTM_IDW_distributed.grd'&
               &,form='unformatted'&
                 &,access='direct',recl=4&
                  &,action='read',status='old',convert='little_endian')                  
                 do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
                      read(116,rec=(igu-1)*210+jgu) mddom%wgg(jgu,igu)
                 enddo
				enddo
       close(116)
...
open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/SF/&
        &TP_SF_memoryfree.grd'&        
               &,form='unformatted'&
		...
		do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
		   do chlguii=1,360
                      read(116,rec=(((jgu-1)*132+igu-1)*360+chlguii)) mddom%SFgg(jgu,igu,chlguii,:)
                    !  print*, mddom%SFgg(jgu,igu,chlguii,:)
					  enddo
                enddo
				enddo
       close(116)
...
open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/UVWReflect-witharea/&
        &TP_SVFcosAsecA_SRTM_IDW.grd'&
               &,form='unformatted'&
                 &,access='direct',recl=4&
                  &,action='read',status='old',convert='little_endian')                  
                    !read(116,rec=1)mddom%ugg
				do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
                      read(116,rec=(igu-1)*210+jgu) mddom%asvfgg(jgu,igu)
                 enddo
				enddo
       close(116)
...
open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/UVWReflect-witharea/&
        &TP_Reflect_CtSECA_SRTM_IDW.grd'&
               &,form='unformatted'&
                 &,access='direct',recl=4&
                  &,action='read',status='old',convert='little_endian')                  
                    !read(116,rec=1)mddom%ugg
				do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
                      read(116,rec=(igu-1)*210+jgu) mddom%refgg(jgu,igu)
                 enddo
				enddo
       close(116)
!ChlGu AnHuang Add 3DSTSRE 20210408
...
open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/UVWReflect-witharea/&
        &TP_TACB_SRTM_IDW.grd'&
               &,form='unformatted'&
				...
				do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
                      read(116,rec=(igu-1)*210+jgu) mddom%TACBgg(jgu,igu)
                 enddo
				enddo
       close(116)

open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/UVWReflect-witharea/&
        &TP_TASB_SRTM_IDW.grd'&
               &,form='unformatted'&
                 &,access='direct',recl=4&
                  &,action='read',status='old',convert='little_endian')                  
                    !read(116,rec=1)mddom%ugg
				do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
                      read(116,rec=(igu-1)*210+jgu) mddom%TASBgg(jgu,igu)
                 enddo
				enddo
       close(116)
...
open(116,file='/data1/chlgu_data1/regcm/3Darea_Ebalance/RADmodi/UVWReflect-witharea/&
        &TP_SECA_SRTM_IDW.grd'&
               &,form='unformatted'&
				...
				do jgu= jde1,jde2   !x   west2east
                  do igu=ide1,ide2  !y   south2north
                      read(116,rec=(igu-1)*210+jgu) mddom%SECAgg(jgu,igu)
                 enddo
				enddo
       close(116)
!ChlGu AnHuang Add 3DSTSRE 20210917       
