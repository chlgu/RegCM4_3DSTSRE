...
!
    ...
    real(rk8) , pointer , dimension(:) :: alb , albc , emiss , &
            ...
            asw , alw , abv , sol , czen !, czen_gu !ChlGu AnHuang Add 3DSTSRE add czen_gu cAz
    ...
    logical , pointer , dimension(:) , intent(in) :: czengt0 ,czengt0_gu !ChlGu AnHuang Add 3DSTSRE add czengt0_gu

  real(rk4) , pointer , dimension(:) ::  cref, casvf, cSF !add by chlgu 20211123
  real(rk4) , pointer , dimension(:) ::  cTACB, cTASB, cSECA, cAZsolar !add by chlgu 20211123
...
!
    ...
    if ( dosw ) then
      ...
      call radcsw(n1,n2,pnm,h2ommr,o3mmr,cld,clwp,rel,rei,fice,eccf,  &
                  ...
                  outtaucl,outtauci, &
			cSF,cTACB,cTASB,cSECA,cAZsolar,casvf,cref,czengt0_gu) !ChlGu AnHuang Add 3DSTSRE 20211123				  				  
                 ! czen_gu,czengt0_gu,cref,casvf,cSF) !ChlGu AnHuang Add 3DSTSRE 20210219
      !
...
!
    ...
    real(rk8) , pointer , dimension(:) :: aeradfo , aeradfos , fsds , fsnirt , &
             ...
             alw , abv , sol , czen!, czen_gu !ChlGu AnHuang Add 3DSTSRE 20210219 add czen_gu
    logical , pointer , dimension(:) , intent(in) :: czengt0 ,czengt0_gu !ChlGu AnHuang Add 3DSTSRE add czengt0_gu
   ! real(rk4) , pointer , dimension(:) :: cref,casvf,cSF !ChlGu AnHuang Add 3DSTSRE 20210219
  real(rk4) , pointer , dimension(:) ::  cref, casvf, cSF !add by chlgu 20211123
  real(rk4) , pointer , dimension(:) ::  cTACB, cTASB, cSECA, cAZsolar !add by chlgu 20211123
real(rk8)  dirstmp,difstmp,dirltmp,difltmp,czentmp,swdirterrain !add by chlgu 20211123
real(rk8)  swdifterrain,lwdirterrain,lwdifterrain  !add by chlgu 20211123
real(rk8) ,allocatable, dimension(:) ::  moditmp,modialbdirsw,modialbdifsw    !ChlGu AnHuang Add 3DSTSRE 20211123
real(rk8) ,allocatable,dimension(:) ::  modialbdirlw,modialbdiflw    !ChlGu AnHuang Add 3DSTSRE 20211123

real(rk8) ,allocatable,dimension(:) :: moditmpdirsw,moditmpdifsw    !ChlGu AnHuang Add 3DSTSRE 20211229
real(rk8) ,allocatable,dimension(:) :: moditmpdirlw,moditmpdiflw    !ChlGu AnHuang Add 3DSTSRE 20211229
real(rk8) ,allocatable,dimension(:) :: diralb1,difalb1 !ChlGu AnHuang Add 3DSTSRE 20220102
...
!
    real(rk8) :: abarii , abarli , bbarii , bbarli , cbarii , cbarli ,  &
            ...
            !  dirtmp, diftmp,czentmp  !ChlGu AnHuang Add 3DSTSRE 20210220
...
!    print*,'czen=',czen(n1),'casvf=',casvf(n2)
    do n = n1 , n2
      if ( czengt0(n) ) then
    ...
    do ns = 1 , nspi
      ...
      ww(indxsl) = ww(indxsl) + d_one
             ...
             allocate (moditmp(n2-n1+1)) !ChlGu AnHuang Add 3DSTSRE
             allocate (modialbdirsw(n2-n1+1) )
             allocate (modialbdifsw(n2-n1+1) )	
             allocate (modialbdirlw(n2-n1+1) )
             allocate (modialbdiflw(n2-n1+1) ) !ChlGu AnHuang Add 3DSTSRE
             allocate (moditmpdirsw(n2-n1+1) )
             allocate (moditmpdifsw(n2-n1+1) )	
             allocate (moditmpdirlw(n2-n1+1) )
             allocate (moditmpdiflw(n2-n1+1) ) !ChlGu AnHuang Add 3DSTSRE 20211229
		     allocate (diralb1(n2-n1+1) )
	         allocate (difalb1(n2-n1+1) )    !ChlGu AnHuang Add 3DSTSRE 20220102
      ...
      do n = n1 , n2
        if ( czengt0(n) ) then
          ...
          !
      if ( wavmid < 0.7D0 ) then
      !   sols(n) = sols(n) + exptdn(n,kzp1)*solflx(n)*d_r1000  !RegCM4.5  origin code
      !   solsd(n) = solsd(n) + &                                     !RegCM4.5  origin code
       !  (fluxdn(n,kzp1)-exptdn(n,kz + 1))*solflx(n)*d_r1000        !RegCM4.5  origin code
		   
   dirstmp=exptdn(n,kzp1)*solflx(n)*d_r1000                     !ChlGu AnHuang Add 3DSTSRE
   difstmp=(fluxdn(n,kzp1)-exptdn(n,kz + 1))*solflx(n)*d_r1000   !ChlGu AnHuang Add 3DSTSRE
	!  print*,'solflx(n),dirstmp,difstmp',solflx(n),dirstmp,difstmp
  czentmp=dmax1(1.0D-3,czen(n))    !ChlGu AnHuang Add 3DSTSRE
   swdirterrain=(1-0.04806*(1-cSF(n))) * dirstmp/czentmp*&
	 (czentmp+  cTACB(n)*sqrt(1-czentmp**2)*cos(cAZsolar(n))  + &
	 cTASB(n)*sqrt(1-czentmp**2)*sin(cAZsolar(n))  )/cSECA(n)   !ChlGu AnHuang Add 3DSTSRE

      if (czengt0_gu(n)) then        
swdifterrain=difstmp*(swdirterrain/( solflx(n)/(eccf*czen(n)) )&
+1.0*casvf(n)*(1.0-dirstmp/( solflx(n)/(eccf*czen(n)) ))/cSECA(n)) + &  
                  dirstmp*adirsw(n)*cref(n)/cSECA(n)+ &
                  difstmp*adifsw(n)*cref(n)/cSECA(n)       !ChlGu AnHuang Add 3DSTSRE  !should check what is casvf,dose it already include 0.5
		 else
swdifterrain=difstmp*(1.0*casvf(n)&
*(1.0-dirstmp/( solflx(n)/(eccf*czen(n)) )) /cSECA(n))+ &    
                  dirstmp*adirsw(n)*cref(n)/cSECA(n)+ &     
                  difstmp*adifsw(n)*cref(n)/cSECA(n)       !ChlGu AnHuang Add 3DSTSRE         	  
         endif
         swdirterrain=dmax1(0.0,swdirterrain)   !ChlGu AnHuang Add 3DSTSRE
         swdifterrain=dmax1(0.0,swdifterrain)    !ChlGu AnHuang Add 3DSTSRE

 		sols(n) = sols(n) + swdirterrain	  !ChlGu AnHuang Add 3DSTSRE	   
		solsd(n) = solsd(n) + swdifterrain	    !ChlGu AnHuang Add 3DSTSRE
	   
		moditmp(n) = (swdirterrain+swdifterrain)/dmax1((dirstmp+difstmp),0.1D-3)  !ChlGu AnHuang Add 3DSTSRE
		!modialbdirsw(n) = ( dirstmp - swdirterrain ) / dmax1(dirstmp,0.1D-3)   !ChlGu AnHuang Add 3DSTSRE
		!modialbdifsw(n) = ( difstmp - swdifterrain ) / dmax1(difstmp,0.1D-3)   !ChlGu AnHuang Add 3DSTSRE	

	 modialbdirsw(n) = ( dirstmp - swdirterrain ) / dmax1(swdirterrain,0.1D-3)  !ChlGu AnHuang Add 3DSTSRE 20211229
	 modialbdifsw(n) = ( difstmp - swdifterrain ) / dmax1(swdifterrain,0.1D-3)  !ChlGu AnHuang Add 3DSTSRE 20211229
		moditmpdirsw(n) =  swdirterrain / dmax1(dirstmp,0.1D-3)   !ChlGu AnHuang Add 3DSTSRE 20211229
		moditmpdifsw(n) =  swdifterrain / dmax1(difstmp,0.1D-3)   !ChlGu AnHuang Add 3DSTSRE 20211229	
		
				   
            !KN         added below
         !   abv(n) = abv(n) + (solflx(n) *            &         !RegCM4.5  origin code
           !            (fluxdn(n,kzp1)-fluxup(n,kz + 1)))*  &      !RegCM4.5  origin code
           !            (d_one-asw(n))/(d_one-diralb(n))*d_r1000      !RegCM4.5  origin code
            !KN         added above
  
		   !ChlGu AnHuang Add 3DSTSRE! put it after recalculate fluxdn and fluxup
	
			
			
          else
          !  soll(n) = soll(n) + exptdn(n,kzp1)*solflx(n)*d_r1000   !RegCM4.5  origin code
          !  solld(n) = solld(n) + &                                    !RegCM4.5  origin code
          !         (fluxdn(n,kzp1)-exptdn(n,kz + 1))*solflx(n)*d_r1000   !RegCM4.5  origin code
   
       dirltmp=exptdn(n,kzp1)*solflx(n)*d_r1000       !ChlGu AnHuang Add 3DSTSRE
        difltmp=(fluxdn(n,kzp1)-exptdn(n,kz + 1))*solflx(n)*d_r1000     !ChlGu AnHuang Add 3DSTSRE
        czentmp=dmax1(1.0D-3,czen(n))                               !ChlGu AnHuang Add 3DSTSRE
 ! print*,'solflx(n),dirltmp,difltmp',solflx(n),dirltmp,difltmp
     lwdirterrain=(1-0.04806*(1-cSF(n))) * dirltmp/czentmp&
	 *(czentmp+  cTACB(n)*sqrt(1-czentmp**2)*cos(cAZsolar(n))  + &
	 cTASB(n)*sqrt(1-czentmp**2)*sin(cAZsolar(n)))/cSECA(n)	      !ChlGu AnHuang Add 3DSTSRE		
		
  if ( czengt0_gu(n)) then        
lwdifterrain=difltmp*(lwdirterrain/( solflx(n)/(eccf*czen(n)) )&
+1.0*casvf(n)*(1.0-dirltmp/( solflx(n)/(eccf*czen(n)) ))/cSECA(n)) &  
                 + dirltmp*adirlw(n)*cref(n)/cSECA(n) &
                  +difltmp*adiflw(n)*cref(n)/cSECA(n)   !ChlGu AnHuang Add 3DSTSRE  !should check what is casvf,dose it already include 0.5
         else
lwdifterrain=difltmp*(1.0*casvf(n)&
*(1.0-dirltmp/( solflx(n)/(eccf*czen(n)) ))&
     /cSECA(n))+dirltmp*adirlw(n)*cref(n)/cSECA(n)+ &
                  difltmp*adiflw(n)*cref(n)/cSECA(n)        !ChlGu AnHuang Add 3DSTSRE    
         endif
         lwdirterrain=dmax1(0.0,lwdirterrain)        !ChlGu AnHuang Add 3DSTSRE
         lwdifterrain=dmax1(0.0,lwdifterrain)          !ChlGu AnHuang Add 3DSTSRE
         soll(n) = soll(n) + lwdirterrain         !ChlGu AnHuang Add 3DSTSRE
         solld(n) = solld(n) +  lwdifterrain        !ChlGu AnHuang Add 3DSTSRE   

		moditmp(n) = (lwdirterrain+lwdifterrain)/dmax1((dirltmp+difltmp),0.1D-3)     !ChlGu AnHuang Add 3DSTSRE


 	modialbdirlw(n) = ( dirltmp - lwdirterrain ) / dmax1(lwdirterrain,0.1D-3) !ChlGu AnHuang Add 3DSTSRE 20211229  
 	modialbdiflw(n) = ( difltmp - lwdifterrain ) / dmax1(lwdifterrain,0.1D-3) !ChlGu AnHuang Add 3DSTSRE 20211229  

		moditmpdirlw(n) =  lwdirterrain / dmax1(dirltmp,0.1D-3) !ChlGu AnHuang Add 3DSTSRE 20211229    
		moditmpdiflw(n) =  lwdifterrain / dmax1(difltmp,0.1D-3) !ChlGu AnHuang Add 3DSTSRE 20211229
	  

        !	 fsnirtsq(n) = fsnirtsq(n) + solflx(n)*(fluxdn(n,0)-fluxup(n,0))  
		 ! put it after recalculate fluxdn and fluxup
			
			
			
            !KN         added below
       !     abv(n) = abv(n) + &                                !RegCM4.5  origin code
         !              (solflx(n)*(fluxdn(n,kzp1)-fluxup(n,kz + 1)))* &       !RegCM4.5  origin code
         !               (d_one-alw(n))/(d_one-diralb(n))*d_r1000      !RegCM4.5  origin code
            !KN         added above
...
!!!!!!ChlGu AnHuang Add 3DSTSRE added bellow  for energy balance  20211123 !!!!!!!!!!!!!!!!!!!!!!!!
!!!!! effective albedo including the terrain radiative effects
      if ( wavmid < 0.7D0 ) then
  
          if ( czengt0(n) ) then
     !       diralb(n) = adirsw(n) + modialbdirsw(n) ! effective albedo including the terrain radiative effects
     !       difalb(n) = adifsw(n) + modialbdifsw(n) ! effective albedo including the terrain radiative effects
        !ChlGu AnHuang Add 3DSTSRE 20220102
         diralb1(n)=diralb(n)
         difalb1(n)=difalb(n)
      !revise EFF ALB in 20211229 chlgu      
      diralb1(n) = moditmpdirsw(n)*(adirsw(n) + modialbdirsw(n)) ! effective albedo including the terrain radiative effects
      difalb1(n) = moditmpdifsw(n)*(adifsw(n) + modialbdifsw(n)) ! effective albedo including the terrain radiative effects
       diralb1(n)= dmax1( dmin1( diralb1(n) ,1.0) ,0.0)
	   difalb1(n)= dmax1( dmin1( difalb1(n) ,1.0) ,0.0)
	  !revise EFF ALB in 20211229 chlgu
      ...
      ! Wavelength greater than 0.7 micro-meter
          ...
          if ( czengt0(n) ) then
      !      diralb(n) = adirlw(n) + modialbdirlw(n) ! effective albedo including the terrain radiative effects
       !     difalb(n) = adiflw(n) + modialbdiflw(n) ! effective albedo including the terrain radiative effects
         !ChlGu AnHuang Add 3DSTSRE 20220102
         diralb1(n)=diralb(n)
         difalb1(n)=difalb(n)      
      !revise EFF ALB in 20211229 chlgu
       diralb1(n) = moditmpdirlw(n)*(adirlw(n) + modialbdirlw(n)) ! effective albedo including the terrain radiative effects
       difalb1(n) = moditmpdiflw(n)*(adiflw(n) + modialbdiflw(n)) ! effective albedo including the terrain radiative effects
       diralb1(n)= dmax1( dmin1( diralb1(n) ,1.0) ,0.0)
	   difalb1(n)= dmax1( dmin1( difalb1(n) ,1.0) ,0.0) 
      !revise EFF ALB in 20211229 chlgu
	   end if
        ...
        if ( czengt0(n) ) then
          rupdir(n,kzp1) = diralb1(n)
          rupdif(n,kzp1) = difalb1(n)
        end if
...
!!!!! effective albedo including the terrain radiative effects	  
!!!!!!ChlGu AnHuang Add 3DSTSRE added above  for energy balance  20211123 !!!!!!!!!!!!!!!!!!!!!!!!

		!ChlGu AnHuang Add 3DSTSRE put abv here now
      if ( wavmid < 0.7D0 ) then
        ...
        !ChlGu AnHuang Add 3DSTSRE put abv above now		   
		
	 

	 if ( wavmid .ge. 0.7D0 ) then
	 fsnirtsq(n) = fsnirtsq(n) + solflx(n)*(fluxdn(n,0)-fluxup(n,0))
     endif		  
...
if(k.eq.kz) then
fswdn(n,k+1) = fswdn(n,k+1) + solflx(n)*fluxdn(n,k+1)*moditmp(n) !ChlGu AnHuang Add 3DSTSRE
else
fswdn(n,k+1) = fswdn(n,k+1) + solflx(n)*fluxdn(n,k+1)
endif               
         end if
...
!          sol(n) = solflx(n)*d_r1000*fluxdn(n,kzp1)
           sol(n) = solflx(n)*d_r1000*fluxdn(n,kzp1)*moditmp(n)  !ChlGu AnHuang Add 3DSTSRE20211124 
           end if
        end do
      end if
      !FAB
      ...
      if ( ichem == 1 .and. idirect > 0 ) then
        ...
        do n = n1 , n2
          if ( czengt0(n) ) then
             rupdir(n,2) = diralb(n) 
             rupdif(n,2) = difalb(n) 
             rupdir(n,2) = diralb1(n)  !ChlGu AnHuang Add 3DSTSRE 20220102
             rupdif(n,2) = difalb1(n)	!ChlGu AnHuang Add 3DSTSRE 20220102 	                            
			
	!already include the terrainRADeffects  by the modified albedo chlgu 			
          end if
        end do
        do k = 1 , 0 , -1
          do n = n1 , n2
	...
	!already include the terrainRADeffects  by the modified albedo chlgu 		  
      ...
      !
	         deallocate (moditmp) !ChlGu AnHuang Add 3DSTSRE
             deallocate (modialbdirsw) !ChlGu AnHuang Add 3DSTSRE 
             deallocate (modialbdifsw) !ChlGu AnHuang Add 3DSTSRE 	
             deallocate (modialbdirlw) !ChlGu AnHuang Add 3DSTSRE 
             deallocate (modialbdiflw) !ChlGu AnHuang Add 3DSTSRE

             deallocate (moditmpdirsw) !ChlGu AnHuang Add 3DSTSRE 
             deallocate (moditmpdifsw) !ChlGu AnHuang Add 3DSTSRE 	
             deallocate (moditmpdirlw) !ChlGu AnHuang Add 3DSTSRE 
             deallocate (moditmpdiflw) !ChlGu AnHuang Add 3DSTSRE
             deallocate (diralb1) !ChlGu AnHuang Add 3DSTSRE
		     deallocate (difalb1) !ChlGu AnHuang Add 3DSTSRE	 
    ...
    do n = n1 , n2
      fsds(n) = fswdn(n,kzp1)
!fsds should not be modified because fswdn(n,kzp1) has been modified. chlgu
     end do
#ifdef DEBUG
    call time_end(subroutine_name,indx)
#endif
