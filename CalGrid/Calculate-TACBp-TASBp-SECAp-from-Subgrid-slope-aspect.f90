!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!To calculate Gridscale TACBp TASBp SECAp
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          real tmpu(6000,6000),tmpv(6000,6000),tmpw(6000,6000)
          real potu(320,240),potv(320,240),potw(320,240)
          ...
          real sumrd,sumtmpu,sumtmpv,sumtmpw
       ...
       open(53,file='E:/CORDEX-domain/xlat.dat') !latitude of each model grids
         do j=1,240
 	   	read(53,*) latp(:,j)
         enddo 
        close(53)
       ...
       open(55,file='E:/CORDEX-domain/xlon.dat')  !longitude of each model grids
         do j=1,240
 	   	read(55,*) lonp(:,j)
         enddo         
        close(55)
...
!              open file and select sub-point
                 ...
                 do mj=numlat2,numlat1
                  ...
                  write(cmj,'(i2)') mj
                   ...
                   inquire(file='E:/SRTM_3D/1srtm_h/alfa1'//fname//'.grd',exist=alive)
                    ...
                    open(116,file='E:/SRTM_3D/1srtm_h/beta1'//fname//'.grd',&
                     &form='unformatted',access='direct',&
                     &recl=6000*6000*4)  
                    read(116,rec=1)tmpb
                    close(116)
           ...
           print*,'inti1=',inti1,'inti2=',inti2,'intj1=',intj1,'intj2=',intj2
                    ...
                    do ii=inti1,inti2
                      ...
                      if(tmpb(ii,jj).lt.-10.0)  tmpb(ii,jj)=0.0
                      tmpu(ii,jj)=tan(tmpa(ii,jj))*cos(tmpb(ii,jj))
                      tmpv(ii,jj)=tan(tmpa(ii,jj))*sin(tmpb(ii,jj))
                      tmpw(ii,jj)=1.0/cos(tmpa(ii,jj))
                         ...
                         intmpw(numint)=tmpw(ii,jj)/intrd(numint) 
...
!                     !enddo nn
            ...
            potv(i,j)=-999000000.0    
            potw(i,j)=-999000000.0            
 ...
 !         enddo j,endo i  
          ...
          open(51,file='CORDEXEA_TACB_SRTM_IDW.grd',form='unformatted'&
            &,access='direct',recl=320*240*4)
          write(51,rec=1) potu
          close(51)
          
          open(51,file='CORDEXEA_TASB_SRTM_IDW.grd',form='unformatted'&
            &,access='direct',recl=320*240*4)
          write(51,rec=1) potv
          close(51)              
            
          open(51,file='CORDEXEA_SECA_SRTM_IDW.grd',form='unformatted'&
            &,access='direct',recl=320*240*4)
          write(51,rec=1) potw
          close(51) 
